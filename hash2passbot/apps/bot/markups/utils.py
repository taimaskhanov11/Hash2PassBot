...
def get_inline_button(bnt_data) -> InlineKeyboardButton:
    return InlineKeyboardButton(text=bnt_data[0], callback_data=bnt_data[1])


def get_inline_url_button(bnt_data) -> InlineKeyboardButton:
...
def inline_keyboard_builder(keyboard: Iterable[tuple[str, CallbackData | str]]):
    builder = InlineKeyboardBuilder()
    [builder.button(text=i[0], callback_data=i[1]) for i in keyboard]
    return builder
